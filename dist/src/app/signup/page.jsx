...
const signup_1 = __importDefault(require("@/actions/user/authentication/signup"));
function Page() {
    const form = (0, react_hook_form_1.useForm)({
        resolver: (0, zod_1.zodResolver)(user_validation_1.userSignupValidation),
        defaultValues: {
            name: '',
            username: '',
            email: '',
    ...
    function onSubmit(values) {
        return __awaiter(this, void 0, void 0, function* () {
            setError('');
            setLoading(true);
            const res = yield (0, signup_1.default)(values);
            if (res.error) {
                setError(res.error);
                setLoading(false);
                return;
                ...
                <form_1.Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full max-w-[400px] flex-col items-center justify-center gap-7 rounded-2xl bg-white px-5 py-7 shadow-[0px_0px_3px_1px_#00000025]">
                        ...
                        <div className="flex w-full flex-col gap-2">
                            <form_1.FormField control={form.control} name="name" render={({ field }) => {
            return (<form_1.FormItem className="w-full space-y-1">
                                            <div className="flex items-center gap-2 overflow-hidden py-1">
                                                <form_1.FormLabel className="shrink-0 !text-black">Full Name</form_1.FormLabel>
                            ...
                            <form_1.FormField control={form.control} name="username" render={({ field }) => {
            return (<form_1.FormItem className="w-full space-y-1">
                                            <div className="flex items-center gap-2 overflow-hidden py-1">
                                                <form_1.FormLabel className="shrink-0 !text-black">Username</form_1.FormLabel>
                                                <form_1.FormMessage className="truncate text-xs"/>
                                            </div>
                                            <form_1.FormControl>
                                                <input_1.Input placeholder="john123" {...field}/>
                                            </form_1.FormControl>
                                        </form_1.FormItem>);
        }}/>
            ...
            return (<form_1.FormItem className="w-full space-y-1">
                        ...
                        <div className="flex w-full flex-col items-center justify-evenly gap-3">
                            ...
                            <div className="text-sm">
                                Already a mebmer?{' '}
                                <link_1.default href="/login" className="text-blue-600">
                                    Login
                                </link_1.default>
                            </div>
                        </div>

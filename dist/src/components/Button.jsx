...
const buttonVariants = (0, class_variance_authority_1.cva)('inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 dark:focus-visible:ring-slate-300', {
    variants: {
        variant: {
            default: 'bg-indigo-500 text-slate-50 shadow hover:bg-indigo-500/90 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-50/90',
...
const Button = React.forwardRef((_a, ref) => {
    var { className, variant, size, loading, asChild = false, children } = _a, props = __rest(_a, ["className", "variant", "size", "loading", "asChild", "children"]);
    const Comp = asChild ? react_slot_1.Slot : 'button';
    return (<Comp className={(0, utils_1.cn)(buttonVariants({ variant, size, className }))} ref={ref} disabled={loading} {...props}>
            {loading && <cg_1.CgSpinner className="mr-1 animate-spin text-lg"/>}
            {children}
        </Comp>);
});
